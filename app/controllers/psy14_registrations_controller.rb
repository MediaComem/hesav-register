...
class Psy14RegistrationsController < ApplicationController
  ...
  def new
    ...
    if event.open < DateTime.now && event.close > DateTime.now
      @registration = Psy14Registration.new
    else
  ...
  def accepted
    ...
    if !sha_valid(getParams)
      ...
      registration = Psy14Registration.find_by_id(orderID_back)
        if registration == nil
  ...
  private
    ...
    def post_params
      params.require(:psy14_registration).permit(:last_name,:first_name,:type_short_name,:city,:email,:street,:npa,:employer,:job)
    end
