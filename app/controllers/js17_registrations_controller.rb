require 'httparty'
require 'js17_csv_service'

class Js17RegistrationsController < ApplicationController
  ...
  def admin
    ...
    registrations = Js17Registration.where("created_at > :date_start AND payed",{date_start: date_start}).order("created_at DESC").all
    ...
    respond_to do |format|
      ...
      format.csv {
        ...
        csv_string = Js17Csv2Service.generate(registrations)
  ...
  def create
    ...
    event = Event.find_by_short_name!(@event_name)
    @registration = Js17Registration.new(post_params)
    ...
    if ((atelier2_count >= @atelier2_limit) && @registration.type_choice == "atelier2") || ((atelier3_count >= @atelier3_limit) && @registration.type_choice == "atelier3")
      ...
      begin
        charge = Stripe::Charge.create(
          ...
          :amount      => @amount,
          :description => 'Inscription Journée scientifique HESAV 2017',
          :currency    => 'chf'
        )
        @registration.payed = true
        @registration.save
  ...
  private
    ...
    def registration_ok(msg,registration)
      ...
      Js17Mailer.success_email(registration).deliver
    end
    ...
    def payment_not_accepted(msg,params)
      ...
      Js17Mailer.error_email(msg,params).deliver
    end
  protected
    def authenticate
