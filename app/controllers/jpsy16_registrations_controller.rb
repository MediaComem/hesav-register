require 'httparty'
require 'jpsy16_csv_service'

class Jpsy16RegistrationsController < ApplicationController
  ...
  def admin
    ...
    registrations = Jpsy16Registration.where("created_at > :date_start AND payed", {date_start: date_start}).order("created_at DESC").all
    ...
    respond_to do |format|
      ...
      format.csv {
        ...
        csv_string = Jpsy162CsvService.generate(registrations)
  ...
  def new
    ...
    if event.open < DateTime.now && event.close > DateTime.now
      @registration = Jpsy16Registration.new
    else
  ...
  def create
    ...
    event = Event.find_by_short_name!(@event_name)
    @registration = Jpsy16Registration.new(post_params)
  ...
  def accepted
    ...
    if !sha_valid(getParams)
      ...
      if getParams["orderID"] != nil
        ...
        registration = Jpsy16Registration.find_by_id(orderID_back)
        if registration == nil
  ...
  def post_params
    params.require(:jpsy16_registration).permit(:last_name, :first_name, :type_price, :type_choice, :city, :email, :street, :npa, :employer)
  end
  ...
  def registration_ok(msg, registration)
    ...
    Jpsy16Mailer.success_email(registration).deliver
  end
